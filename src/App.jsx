...
const Btn = ({ children, onClick, color, small, disabled }) => (
  <button onClick={onClick} disabled={disabled} style={{
    background: disabled ? "#ccc" : (color || PALETTE.teal), color: "#fff", border: "none",
    borderRadius: 10, padding: small ? "7px 14px" : "11px 20px",
    fontSize: small ? 13 : 15, fontWeight: 600, cursor: disabled ? "not-allowed" : "pointer", fontFamily: "inherit",
  }}>{children}</button>
);
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 16, padding: 16, marginBottom: 14 }}>
      ...
      <LabelRow label="Amount ($)">
        <input type="number" style={inputStyle} placeholder="0" value={amount} onChange={e => setAmount(e.target.value)} inputMode="decimal" />
      </LabelRow>
      <LabelRow label="Note">
        <input type="text" style={inputStyle} placeholder="Description..." value={note} onChange={e => setNote(e.target.value)} />
      </LabelRow>
      <LabelRow label="By">
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 16, padding: 16, marginBottom: 14 }}>
      ...
      <LabelRow label="Nights">
        <input type="number" style={inputStyle} placeholder="1" value={nights} onChange={e => setNights(e.target.value)} inputMode="numeric" />
      </LabelRow>
      <LabelRow label="Guest">
        <input type="text" style={inputStyle} placeholder="Guest name" value={guest} onChange={e => setGuest(e.target.value)} />
      </LabelRow>
...
function parseIcal(text) {
  ...
  for (let i = 1; i < blocks.length; i++) {
    ...
    const parseDate = (s) => { if (!s) return null; const c = s.replace(/T.*/,""); return c.length===8?`${c.slice(0,4)}-${c.slice(4,6)}-${c.slice(6,8)}`:null; };
    const dtstart = parseDate(get("DTSTART")), dtend = parseDate(get("DTEND"));
...
function getEventForDate(d,evs){return evs.find(e=>e.dtstart<=d&&(e.dtend?d<e.dtend:false));}

const api = async (action, body) => {
  const url = `/api/baserow?action=${action}`;
...
export default function App() {
  ...
  // Load data from Baserow on mount
  useEffect(() => {
  ...
  const handleAddExpense = useCallback(async (data) => {
    setSaving(true);
    try { const saved=await api("add_expense",data); setExpenses(prev=>[...prev,{...saved,id:saved.id||Date.now()}]); setShowExpForm(false); }
    catch(e){}
    setSaving(false);
  }, []);
  ...
  const handleAddIncome = useCallback(async (data) => {
    setSaving(true);
    try { const saved=await api("add_income",data); setIncome(prev=>[...prev,{...saved,id:saved.id||Date.now()}]); setShowIncForm(false); }
    catch(e){}
    setSaving(false);
  }, []);

  const handleDelExpense = async (id) => {
    setExpenses(prev=>prev.filter(x=>x.id!==id));
    await api("del_expense",{id});
  };

  const handleDelIncome = async (id) => {
  ...
  const askAI = async () => {
    ...
    const context=`You are a financial assistant for "Poolside Paradise", an Airbnb rental. Owners: Harry & Lily. Partner: Cindy (30%). Income:${fmt(totalIncome)} Expenses:${fmt(totalExpenses)} Profit:${fmt(netProfit)} Cindy:${fmt(cindyShare)} Owners:${fmt(ownerShare)}. Reply in same language as user.`;
    try {
      ...
      setAiHistory([...newHistory,{role:"assistant",content:d.content?.[0]?.text||"Lỗi: "+JSON.stringify(d).slice(0,100)}]);
      ...
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
          {cells.map((d,i)=>{
            ...
            const booked=isBooked(ds,icalEvents),isToday=ds===today,ev=getEventForDate(ds,icalEvents);
      ...
      <div style={{padding:"18px 16px"}}>
        ...
        {tab==="dashboard"&&(
          <div>
            ...
            <div style={{background:"#fff",borderRadius:16,padding:16}}>
              ...
              {catBreakdown.length===0&&<div style={{color:PALETTE.muted,fontSize:13}}>No expenses yet</div>}
        ...
        {tab==="ai"&&(
          <div>
            ...
            <div style={{display:"flex",gap:8}}>
              <input style={{...inputStyle,flex:1}} placeholder="Hỏi gì đó..." value={aiQuery} onChange={e=>setAiQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&askAI()}/>
