...
async function runTool(name, input) {
  ...
  if (name === "add_expense") {
    const { date = today, category = "Other", amount, note = "", by = "Harry" } = input;
    ...
    return { success: true, id: data.id, date, category, amount, note, by };
  }

  if (name === "add_income") {
    const { date = today, amount, nights = 1, guest = "", platform = "Airbnb" } = input;
    ...
    const data = await r.json();
    return { success: true, id: data.id, date, amount, nights, guest, platform };
  }
