...
export default function App() {
  ...
  const askAI = async () => {
    ...
    try {
      ...
      // If AI added data, refresh from Baserow
      if (d.action?.result?.success) {
        const { tool, result } = d.action;
        if (tool === "add_expense") {
          setExpenses(prev => [...prev, { ...d.action.input, id: result.id }]);
        } else if (tool === "add_income") {
          setIncome(prev => [...prev, { ...d.action.input, id: result.id, nights: d.action.input.nights || 1, guest: d.action.input.guest || "", platform: d.action.input.platform || "Airbnb" }]);
        }
      }
